...
import {IERC721TL} from "tl-creator-contracts-3.7.1/erc-721/IERC721TL.sol";
import {SendAndReceiveBase, IERC1155Receiver} from "./lib/SendAndReceiveBase.sol";

/// @title Send and Receive ERC721TL
/// @notice A contract that receives 1155 tokens and mints back an ERC721TL token
...
contract SendAndReceiveERC721TL is SendAndReceiveBase {
    ////////////////////////////////////////////////////////////////////////////
    // Types
    ////////////////////////////////////////////////////////////////////////////
    ...
    struct Settings {
        ...
        uint64 numRedeemed;
        uint64 nextUriIndex;
        string baseUri; // Expected format has no trailing slash and should point to files named `0` through `N`
    }
    ...
    /// @dev `initSettings.baseUri` should not include a trailing slash and should reference a folder containing `0` through `N` files
        ...
        initializer
    {
        ...
        s.baseUri = initSettings.baseUri;
    ...
    function _redeem(address recipient, uint256 numRedeemed) internal override {
        ...
        // mint to recipient
        uint256 nextUriIndex = s.nextUriIndex;
        s.nextUriIndex += uint64(numRedeemed);
        for (uint256 i = 0; i < numRedeemed; ++i) {
            string memory uri = string(abi.encodePacked(s.baseUri, "/", uint256(nextUriIndex).toString()));
            IERC721TL(s.outputContractAddress).externalMint(recipient, uri);
            nextUriIndex += 1;
        }
